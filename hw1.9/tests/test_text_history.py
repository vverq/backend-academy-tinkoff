...
class TextHistoryTestCase(TestCase):
    ...
    def test_get_actions_insert_optimization(self):
        ...
        first_insert, second_insert = actions
        self.assertIsInstance(first_insert, InsertAction)
        self.assertIsInstance(second_insert, InsertAction)

        # first_insert
        self.assertEqual(0, first_insert.from_version)
        self.assertEqual(1, first_insert.to_version)
        self.assertEqual("abcd", first_insert.text)
        self.assertEqual(0, first_insert.pos)

        # second_insert
        self.assertEqual(1, second_insert.from_version)
        self.assertEqual(3, second_insert.to_version)
        self.assertEqual("ef", second_insert.text)
        self.assertEqual(3, second_insert.pos)

    def test_get_actions_replace_optimization(self):
        h = TextHistory()
        h.insert("abcd")
        h.replace("e", 1)
        h.replace("fg", 2)
        ...
        replace_action = actions[0]
        self.assertEqual(1, len(actions))
        self.assertIsInstance(replace_action, ReplaceAction)
        self.assertEqual(1, replace_action.from_version)
        self.assertEqual(3, replace_action.to_version)
        self.assertEqual("efg", replace_action.text)
        self.assertEqual(1, replace_action.pos)

    def test_get_actions_delete_optimization(self):
        h = TextHistory()
        h.insert("abcdef")
        h.delete(1, 1)
        h.delete(1, 2)
        ...
        replace_action = actions[0]
        self.assertEqual(1, len(actions))
        self.assertIsInstance(replace_action, DeleteAction)
        self.assertEqual(1, replace_action.from_version)
        self.assertEqual(3, replace_action.to_version)
        self.assertEqual(3, replace_action.length)
        self.assertEqual(1, replace_action.pos)
